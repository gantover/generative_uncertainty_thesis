...
\begin{itemize}
    ...
    \item Using the same encoder for both the Realism and Rarity filtering metric and the \acs{FPR} evaluation metrics leads to a bias that is likely disadvantageous for methods that do not use the \texttt{Inception-v3} encoder, such as Generative Uncertainty.
\end{itemize}
...
\begin{enumerate}
    ...
    \item The FID computation requires the computation of two covariance matrices $\Sigma_r, \Sigma_g$ of size $2048 \times 2048$, which leads to poor sample efficiency \parencite{jayasumana_rethinking_2024}. \label{enum:fid_critique_sample_efficiency}
...
\autoref{fig:fpr_sample_size} illustrates this phenomenon in \cite{kynkaanniemi_improved_2019}'s experimental setup. When looking into the range $[6000,\dots,12000]$ (to match \cite{jazbec_generative_2025}) FID is by far the most affected. Precision is also affected in a visible way, while Recall appears to be significantly more stable.
...
We now explore how this $\gamma$ affects the \acs{LA} approximation. For that let's go back to the Laplace approximation of the log-posterior, this time expanded around the trained weights $\hat\theta = \theta_{MLE}$:
\begin{equation}
...
We are therefore trying to find a $\gamma$ that's negligible compared to the Hessian while still big enough to stabilize the inversion of the Hessian properly.
...
Grouping the part that's independent of $\theta$ in $c$ and setting $\alpha^2 = 1/2$ allows us to get the exact expression of a term from the  simplified loss (\acs{MSE}).
...
\begin{itemize}
    ...
    \begin{align}
    \Sigma^* &= \sigma^2 I + \frac{1}{\mathrm{M}+1} \sum^\mathrm{M}_{m=0} e_m^2 - (\mu^*)^2
    \end{align}
\end{itemize}

For efficiency reasons, we only consider the diagonal terms of $\Sigma^*$:
\begin{align}
    \Sigma^* &\simeq \sigma^2 I + \text{Diag}\left(\frac{1}{\mathrm{M}+1}\sum^\mathrm{M}_{m=0} e_m^2 - (\mu^*)^2 \right) =: \Sigma^{d*}
\end{align}
...
For a neural network with $P$ parameters, the exact hessian matrix of the \acs{NLL} $H_{NLL}\in \mathbb{R}^{P \times P}$ scales quadratically. For modern architectures with millions of weights, computing, storing, and later inverting the dense matrix $H$ is almost impossible.
