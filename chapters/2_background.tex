...
For this \acs{NLL} to be equal to $\mathcal{L}_{\mathrm{simple}}$, we define:
\begin{align}
...
Finally, to reduce the time needed to compute $\hat{F}$ even further, (\cite{jazbec_generative_2025}, Appendix D) use only $1\%$ of the training data for the \acs{ADM} (the Toy Model will use the full training set).

\subsection{Toy Model Posterior Approximation}
\label{sub:gu_th_toy_model_posterior_approximation}

In this thesis, chapters \ref{ch:reproduce_toy}, \ref{ch:quantifying_gu_filtering_toy}, \ref{ch:toy_exp_posterior} will use a much smaller diffusion \textit{Toy Model} that works with $x \in \mathbb{R}^2$. 
