...
In this section, we define the \acs{FID} \parencite{heusel_gans_2018}, \emph{Precision}, \emph{Recall}, \emph{Realism} \parencite{kynkaanniemi_improved_2019} and Rarity \parencite{han_rarity_2022} metrics. All of these operate in a shared feature space: each image (real or generated) is embedded by a pretrained \texttt{Inception-v3} network, yielding a feature vector $\phi$. We write $\boldsymbol{\Phi}_r = \{\phi_r^i\}$ and $\boldsymbol{\Phi}_g = \{\phi_g^i\}$ for the real and generated embedded feature sets respectively. For later definitions that can take either the real or generated feature set, we use $\boldsymbol{\Phi} \in \{\boldsymbol{\Phi}_r, \boldsymbol{\Phi}_g\}$.

We will first go through the batch scale\footnote{Batch scale refers to metrics that need a whole (large) dataset of generated samples to be computed on.} metrics \acs{FPR} (\acs{FID}, \emph{Precision} and \emph{Recall}) aimed to evaluate the ``quality'' of the generated dataset compared to the real dataset.
We first define the simplest metric: the \acs{FID} from \cite{heusel_gans_2018} which relies on fitting a multivariate Gaussian to the generated and real datasets. Then we introduce two slightly more complex metrics: the ``improved'' \emph{Precision} and \emph{Recall} as presented in \cite{kynkaanniemi_improved_2019} which rely on a k-NN manifold approximation.

We finally define the individual\footnote{Opposite of batch scale, those metrics evaluate a single sample from the generated dataset which opens the door to filtering by assigning a score per sample.} samples metrics: \emph{Realism} \parencite{kynkaanniemi_improved_2019} and \emph{Rarity} \parencite{han_rarity_2022} that will be used as baseline to filter the samples and compare with Generative Uncertainty on the \acs{FPR} metrics. Filtering metrics are used to create subsets $\boldsymbol\Phi_f \subseteq \boldsymbol\Phi_g$ that only keep the $N$ best samples with respect to that filtering metric.

\begin{tcolorbox}[colback=gray!10!white,colframe=gray!50!black,sharp corners]
In this thesis, we use the \texttt{Inception-v3} model to match what was done in \cite{jazbec_generative_2025} while the actual paper introducing the \emph{Precision}, \emph{Recall} and \emph{Realism} metrics \parencite{kynkaanniemi_improved_2019} uses a VGG-16 classifier layer instead. \cite{kynkaanniemi_improved_2019} however also tested their metric with \texttt{Inception-v3} and it lead to very similar results in their experiments.
\end{tcolorbox}
...
The first term penalizes a shift in mean appearance; the second penalizes any mismatch in spread and correlation, capturing both mode collapse (too narrow) and over-dispersion (too broad).
Lower is better.
...
Despite its widespread adoption, \acs{FID} suffers from notable structural weaknesses \parencite{jayasumana_rethinking_2024} among other issues described in \autoref{sub:problems_inception}.
\end{tcolorbox}

\subsection{k-NN Manifold Estimate}

Before going on to define the \emph{Precision}, \emph{Recall}, \emph{Realism} and \emph{Rarity} metrics, we need to properly define what the k-NN Manifold Approximation is.
...
\begin{figure}[!htbp]
    ...
    (b) \emph{Precision} is the probability that a random image from $P_g$ falls within the support of $P_r$. (c) \emph{Recall} is the probability that a random image from $P_r$ falls within the support of $P_g$.}
    \label{fig:bg_precision_recall}
\end{figure}
 
\acs{FID} conflates two independently important aspects of generation quality.
\emph{Precision} (fidelity) asks whether generated images look realistic; \emph{Recall} (diversity) asks whether the generated distribution covers the full variety of real images.
A generator suffering from mode collapse can have high precision but low recall; a generator producing noisy, incoherent samples that happen to span the data space can have high recall but low precision.
...
%
\begin{tcolorbox}[colback=gray!10!white,colframe=gray!50!black,sharp corners]
\emph{Precision} counts the fraction of generated samples that fall inside the real
manifold. \emph{Recall} counts the fraction of real samples covered by the generated manifold.
\end{tcolorbox}

Both metrics make no distributional assumptions, in contrast to FID.

\emph{Precision} and \emph{Recall} are not independent ($\indep$): retaining only the highest-quality generated images usually improves precision but shrinks the coverage of real modes, reducing recall.
...
In the same paper as \emph{Precision} and \emph{Recall}, \cite{kynkaanniemi_improved_2019} introduce \emph{Realism}, a metric that can be used to score individual samples by providing an estimate of how deeply a generated sample penetrates the real manifold. 
It does so by comparing the radius of a real neighborhood to the distance from the generated sample to its center. 
Taking the maximum gives the deepest penetration. 
...
where $\boldsymbol\Phi_r^* \subseteq \boldsymbol\Phi_r$ is a filtered subset of real samples anchors defined below. 
The score gets bigger as the generated sample $\phi_g$ gets closer to $\mathcal{M}_k(\boldsymbol\Phi_r)$ until $R \ge 1$ at which point we have an equivalence $f(\phi_g, \boldsymbol{\Phi}_r) = 1 \iff R(\phi_g, \boldsymbol{\Phi}_r) \ge 1$. 
...
If a sample happens to fall in such a region, it can lead to a highly inaccurate score $R$. To fix this, instead of taking the $\max$ across all $\phi_r \in \boldsymbol{\Phi}_r$, we create a subset $\boldsymbol{\Phi}_r^*$ by taking $\boldsymbol{\Phi}_r$ and removing all $\phi_r$ that have a hypersphere of radius larger than the median radius.

Regarding \emph{Precision} and \emph{Recall}, because it is unlikely that a sample falls in a low density region, even with larger hyperspheres, the proportion of samples affected by this is quite low. Therefore, \emph{Precision} and \emph{Recall} being evaluated on large populations of generated samples, do not need such precautions (according to \cite{kynkaanniemi_improved_2019}).
...
While \emph{Realism} was good at evaluating how close a generated image was to the real data manifold, it did so by rewarding images that stayed in safe, familiar territory.
The \emph{Rarity} score \cite{han_rarity_2022} was introduced to explicitly evaluate the ``uncommonness'' of an individual generated image while still detecting and isolating \acs{OOD} (Out Of Distribution) samples, thereby rewarding models that can generate those edge cases without producing \acs{OOD}.
...
% \end{align}
For a generated sample $\phi_g$, we take every ball $B_k$ penetrated by $\phi_g$ and take the smallest ball radius as Rarity score. The bigger the radius, the less dense the region of the manifold which is what \emph{Rarity} tries to measure, additionally, if $\phi_g$ does not penetrate any ball, we consider it \acs{OOD}. Here is the mathematical formulation:

\begin{equation}
  \mathrm{Rarity}(\phi_{g}, \boldsymbol{\Phi}_r)=
  \begin{cases}
  \min\limits_{\substack{\phi_{r} \in \boldsymbol\Phi_{r} \\ \mathrm{s.t.} \;  \phi_{g} \in B_{k}(\phi_{r},\boldsymbol\Phi_{r})}}
  ||\phi_r - \mathrm{NN}_{k}(\phi_{r},\boldsymbol\Phi_{r})||_2 & \exists \phi_r\\
  \mathrm{O.O.D.} & \not\exists \phi_r
  \end{cases}
  \label{eq:rarity}
\end{equation}
...
\emph{Realism} and \emph{Rarity} are thus complementary: \emph{Realism} rewards proximity to the real manifold, while \emph{Rarity} rewards distance from its high-density core. 
In \cite{jazbec_generative_2025} however, they aim to remove artifacts out of the dataset. They hypothesize that those artifacts are located in the lower density regions meaning they use the rarity score for filtering by keeping the images with \textbf{lowest} \emph{Rarity}.
...
The effect of sample size $|\boldsymbol\Phi|$ has been studied for both \acs{FID} \parencite{jayasumana_rethinking_2024} and \emph{Precision}, \emph{Recall} \parencite{kynkaanniemi_improved_2019}.
...
\begin{enumerate}
    ...
    \item Because Precision and Recall rely on the k-NN manifold approximation, having a smaller dataset size $|\boldsymbol\Phi|$ implies a lower density of points across the semantic space making the distance to the $k$-th neighbor bigger and therefore the hypersphere associated to each point bigger, it becomes easier for points of each either $\boldsymbol\Phi_{r,g}$ to fall into the manifold associated to the other distribution $\mathcal{M}_k(\boldsymbol\Phi_{g,r})$, artificially increasing the \emph{Precision} and Recall scores.
\end{enumerate}

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.5\linewidth]{figures/background/fpr_sample_size.png}
    \caption{Taken from \cite{kynkaanniemi_improved_2019}, This figure shows what happens when reducing the sample size $N$ to the \acs{FPR} metrics. \acs{FID} is clearly the most affected, followed by \emph{Recall} and \emph{Precision}}
    \label{fig:fpr_sample_size}
\end{figure}

\autoref{fig:fpr_sample_size} illustrates this phenomenon in \cite{kynkaanniemi_improved_2019}'s experimental setup. When looking into the range $[6000,\dots,12000]$ (to match \cite{jazbec_generative_2025}) \acs{FID} is by far the most affected. \emph{Recall} is also affected in a visible way, while \emph{Precision} appears to be significantly more stable. A further interpretation of what is happening with \emph{Precision}, \emph{Recall} and the effect of the asymmetry $\boldsymbol{\Phi}_r \neq \boldsymbol{\Phi}_g$ is available in appendix \autoref{app:fpr_inflation}.
...
The Gaussian assumption in \acs{FID} has also been critiqued \parencite{jayasumana_rethinking_2024}: ``approximating both the real and generated image distribution by a multivariate Gaussian is provably incorrect''.
...
\begin{itemize}
    ...
    \item \acs{FID} contradicts human judgement \parencite{jayasumana_rethinking_2024}.
    \item Using the same encoder for both \acs{FPR} and the \emph{Realism}/\emph{Rarity} baselines is disadvantageous for methods that do not use the \texttt{Inception-v3} encoder, such as Generative Uncertainty (\acs{GU}).
...
Conceptually, this diffusion process goes in two directions. First, a forward process systematically destroys the structure of the data by adding random noise until it becomes pure noise (matching $p_{\mathrm{init}}(z)$). Secondly, we train a neural network to reverse the process by removing (reversing) one noise step at a time, effectively learning how to generate data from pure noise. 

This framework is versatile enough to be applied to varying tasks, 
from a simple 2D synthetic dataset featuring 25 distinct Gaussian modes (e.g., the \textit{Toy Experiment} in \autoref{ch:reproduce_toy}), to high-dimensional datasets like ImageNet $128 \times 128$ using advanced architectures such as the \acs{ADM} (\autoref{ch:reproduce_adm} experiment) \cite{jazbec_generative_2025}.

\begin{figure}[!htbp]
    \centering
    \includegraphics[width=0.9\linewidth]{figures/theory/diffusion_markov.png}
    \caption{Illustration from \cite{ho_denoising_2020} of the forward and reverse Markov chains in the diffusion framework. The forward process (bottom, right to left) progressively corrupts an initial data sample $x_0 \sim p_{\mathrm{data}}(x)$ with Gaussian noise over $T$ steps until it approaches an isotropic Gaussian distribution $x_T \sim p_{\mathrm{init}}(z)$. The reverse process (top, left to right), parameterized by a learned neural network denoiser $\epsilon_\theta$, iteratively removes the noise step by step to recover a sample from the target data distribution.}
...
\end{equation}
This result is obtained by recursively applying the Gaussian reparameterization trick and merging the independent noise increments (the full step-by-step derivation is given in \autoref{app:diffusion_forward}). We can now sample $x_t$ directly from $x_0$:
\begin{equation}
...
Reversing the forward process exactly is intractable, since the ideal reverse step $p_\theta(x_{t-1}|x_t)$ depends on the entire data distribution. Following \textcite{sohl2015thermo} and \textcite{ho_denoising_2020}, the network is instead trained by minimizing the variational bound $\mathcal{L}_{\mathrm{VLB}}$ (VLB), an upper bound on the negative data log-likelihood; equivalently, $-\mathcal{L}_{\mathrm{VLB}}$ is the evidence lower bound (ELBO) on $\log p_\theta(x_0)$. \textcite{ho_denoising_2020} rewrite this bound as a sum of terms that can each be evaluated in closed form:
\begin{multline}
...
% Note that here the timestep is drawn uniformly, $t \sim \mathrm{Uniform}\{1,\dots,T\}$; the $t=1$ sample reuses this same noise-prediction form in place of the reconstruction term $L_0$, so the single objective covers every reverse step down to $x_1 \to x_0$. 
...
\subsection{Sampling}
After the network $\epsilon_{\hat{\theta}}$ is trained (here $\hat{\theta}$ denotes the trained weights, i.e.\ the optimum of \autoref{eq:simple_loss}), the generative model synthesizes data by sequentially applying $T$ single-step transformations starting from pure noise $x_T \sim \mathcal{N}(0, I)$. Two paradigms to do so are used in this thesis; \textcite{song_denoising_2022} define a single generalized reverse step for both:
\begin{equation}
...
\end{aligned}
\end{equation}
where $\omega \sim \mathcal{N}(0, I)$ and the variance $\sigma_t$ interpolates between the two regimes (the full derivation is provided in \autoref{app:diffusion_sampling}). The two paradigms used in the thesis governing this generation phase:
...
As established in the previous section, exact Bayesian inference requires integrating over the entire parameter space, a task that is intractable for the high-dimensional weight spaces of modern deep neural networks. The Laplace approximation (\acs{LA}) offers a mathematically elegant and computationally feasible post-hoc alternative. Rather than tracking the full posterior during training, the \acs{LA} constructs a local, multivariate Gaussian approximation of the true posterior distribution centered around the pre-trained Maximum a Posteriori (\acs{MAP}) point estimate, denoted here\footnote{Later, we will also denote $\hat{\theta}$ as the pre-trained model's weights, which is a \acs{MLE} (Maximum Likelihood Estimation) instead.} as $\hat{\theta}$.


\subsection{Mathematical Formulation}
The derivation relies on a second-order Taylor series expansion of the log-posterior $\log p(\theta | \mathcal{D})$, evaluated at the mode $\hat{\theta}$. Because $\hat{\theta}$ represents a local maximum of the posterior density (\acs{MAP}), the first-order derivative (the gradient) evaluates to zero. This simplifies the expansion to:
\begin{equation}
...
\end{equation}
By exponentiating this quadratic approximation of the log-density, we find that the approximate posterior takes the exact functional form of a Gaussian distribution. which yields:
\begin{equation}
...
In practice the marginal likelihood $p(\mathcal{D})$ contained in the log-posterior is often intractable. Thankfully, it naturally disappears when computing the Hessian:

% Computing the Hessian $H$ =

% The reason we can develop the approximation on the \textbf{unnormalized} log-posterior therefore ignoring the usually intractable marginal likelihood $p(\mathcal{D})$ or equivalently develop the approximation on the joint distribution $p(\theta, \mathcal{D})$ is that it naturally disappears when computing the Hessian:
\begin{align}
...
Because of the independence ($\indep$) of $p(\mathcal{D})$ from $\theta$, its gradient w.r.t. $\theta$ is $\boldsymbol{0}$.

The marginal likelihood also appears in the $\log p(\hat\theta | \mathcal{D})$ term of the Taylor expansion, but once again, because it is a constant with respect to $\theta$ and does not affect the shape of the posterior, one can simply ignore it and normalize the Gaussian afterwards.

\subsection{Pre-trained models use the MLE}
\label{sub:la_trained_on_mle}

\paragraph{The Problem}
The \acs{MAP} estimate is obtained by optimizing the posterior $p(\theta|\mathcal{D}) \propto p(\mathcal{D}|\theta)p(\theta)$. For the \acs{LA} to be defined properly, it needs to be centered on the \acs{MAP} estimate $\theta_{\text{MAP}}$. Unfortunately, most diffusion models are trained to optimize the likelihood $p(\mathcal{D} | \theta)$ only, leading to a Maximum Likelihood Estimate $\theta_{\mathrm{MLE}}$ (\acs{MLE}) instead. This makes the above derivation mathematically false (the first order derivative is not zero).
...
$\gamma = 1.0$ for the \acs{ADM}. For the Toy model however we will explore the influence of $\gamma$ on a large range of values and document its effect in depth.
...
This chapter builds on \autoref{ch:background} to formalize the computation of generative uncertainty (\acs{GU}), detailing the progression from the posterior approximation to the per-image uncertainty score.
...
\begin{enumerate}
    ...
    \item We are now ready to generate images. We sample a single, fixed noise sample $z \sim \mathcal{N}(0,I)$ that we feed through the base model as well as the M sampled models (everyone gets the same noise sample). This gives us $\mathrm{M}+1$ generated images that only differ by the model weights used. 
    ...
    \item We keep the base model's generated image and report the computed uncertainty score.
\end{enumerate}

\begin{tcolorbox}[colback=gray!10!white,colframe=gray!50!black,sharp corners]
The intuition here is ``if a small perturbation in the models weights changes the meaning/structure of an image drastically between multiple samples, the model is highly uncertain about its output and should assign it a high uncertainty''.
\end{tcolorbox}

\section{Bayesian Model}
\label{sec:bayesian_model}

In this section, we formalize the definition of the posterior predictive distribution in both image (pixel) and semantic vector space which is later used to compute the uncertainty. 

\begin{tcolorbox}[colback=blue!10!white,colframe=blue!50!black,sharp corners]
Because we are not yet trying to compute the uncertainty, in this section we will only define the full analytical form of the posterior predictive distribution which will later be approximated with the Monte Carlo \acs{MC} method to make the uncertainty computation feasible (\autoref{sec:bg_gu_uncertainty_computation_mc}).
\end{tcolorbox}
...
By defining an effective dataset of noise-prediction tasks:

\begin{equation}
\mathcal{D}_{noise} = \{(x_{t,i}, t_i, \epsilon_i)\}_{i=1}^N
\end{equation}

Each datapoint is built from the original training set: we draw a clean image $x_{0,i}$, pick a timestep $t_i$, and corrupt the image with noise $\epsilon_i$ through the forward relation \eqref{eq:forward_reparam} to obtain the intermediate noisy image $x_{t,i}$. The pair $(x_{t,i}, t_i)$ then serves as the regression input and the injected noise $\epsilon_i$ as the target the network is trained to predict. The clean image $x_{0,i}$ itself need not be stored: at a fixed $t_i$ the same forward relation ties $(x_{0,i}, x_{t,i}, \epsilon_i)$ together, so any two of them determine the third, and predicting $\epsilon_i$ is equivalent to recovering $x_{0,i}$. We transform the formulation into an exact Bayesian regression problem. The likelihood of observing this whole effective dataset becomes:
...
For this \acs{NLL} to reduce to $\mathcal{L}_{\mathrm{simple}}$ (up to normalization), we define the noise-prediction likelihood\footnote{Here $s^2$ is an \emph{assumed} observation-noise variance on the regression residual, i.e.\ a modeling choice, not a property of the data. Given $x_{t,i}$, $t_i$ and the clean image, the target $\epsilon_i$ is in fact determined exactly through the forward relation \eqref{eq:forward_reparam}; we nonetheless treat it as a Gaussian target with a constant variance we are free to fix. Setting $s^2 = 1/2$ below is precisely what recovers the unweighted \acs{MSE} constant of $\mathcal{L}_{\mathrm{simple}}$.}:
\begin{align}
...
\cite{jazbec_generative_2025} was based on \cite{kou_bayesdiff_2024} (BayesDiff). Two main contributions were made by \cite{jazbec_generative_2025}. The first one was adding a pre-trained external semantic encoder to evaluate uncertainty in its latent space rather than in pixel-space. The second was to only evaluate uncertainty on the last step of the reverse diffusion process (final image output). The second likely being a direct consequence of the first one because the external encoder is not made to process noisy images.

While it is true that a proper Bayesian framework should track uncertainty across all reverse diffusion timesteps, We show much later in the thesis (\autoref{sub:internal-enc-abla-exp}, using an internal encoder that understands intermediate noisy images in the reverse diffusion process) that the final generated image $x_0$ is the best timestep at which to extract the uncertainty with respect to the evaluation metrics defined in this thesis. 
So from a pure performance standpoint, it seems like we can rely on tracking uncertainty in the final timestep $t=0$ only.

\cite{gupta_quantifying_2026} (FLARE) also argue that not tracking uncertainty across all reverse diffusion timesteps prevents us from properly separating epistemic uncertainty\footnote{Uncertainty in the model's knowledge} from aleatoric uncertainty\footnote{Uncertainty stemming from inherent noise in the reverse diffusion process and data}. We argue however that fixing a noise seed for both the starting noise $z$ and stochastic noise injection $\omega$ for \acs{DDPM}s allows us to properly isolate epistemic uncertainty.
...
\end{align}
Clearly, this integral is intractable. A simple method to go around this limitation is using a Monte Carlo (\acs{MC}) approximation. And the integral becomes
\begin{align}
...
\begin{tcolorbox}[colback=orange!10!white,colframe=orange!50!black,sharp corners]
Throughout this thesis, we will always use the entropy expression without computing the constant term. This is because we are only interested in ordering the uncertainty scores of different samples for filtering purposes.
\end{tcolorbox}

As a side note, this \acs{MC} approximation of the Bayesian posterior objective using a finite ensemble of models makes some other non-Bayesian methods integrate seamlessly in our framework. For instance, in the \textit{Toy Experiment} we first use a Deep Ensemble (\acs{DE}) of models which is obtained by training multiple models with different random seeds.
...
To make the computation tractable, \cite{jazbec_generative_2025} first approximate the curvature using the empirical Fisher Information Matrix (\acs{FIM}).
...
\end{equation}
The factor $N$ converts the per-sample averaged Fisher $\hat F$ into the curvature of the full-dataset \acs{NLL} (\autoref{eq:nll_hessian_scale}); equivalently, $N\hat F$ is just the empirical Fisher \emph{summed} over the $N$ samples rather than averaged. The prior precision $\gamma$ is added on top: because the data term scales with $N$ while the prior does not, $\gamma$ is hopefully small relative to the likelihood curvature, which is exactly the regime in which the \acs{MLE}~$\simeq$~\acs{MAP} approximation of \autoref{sub:la_trained_on_mle} would be justified.
...
In \autoref{ch:toy_exp_posterior}, we still show that despite the criticism, the \acs{LLLA} can be a very capable approximation to filter \acs{OOD} samples for a \textit{Toy Model}.
\end{tcolorbox}
...
Additionally and very importantly, because the Toy Model output space is only 2D, we don't need to use a semantic encoder to compute the uncertainty $c_\phi(x)=x$. 
