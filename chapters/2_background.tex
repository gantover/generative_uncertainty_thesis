...
from a simple 2D synthetic dataset featuring 25 distinct Gaussian modes (e.g., the Toy experiment in \autoref{ch:reproduce_toy}), to high-dimensional datasets like ImageNet $128 \times 128$ using advanced architectures such as the \acs{ADM} (e.g., the \acs{ADM} experiment in \autoref{ch:reproduce_adm}) \cite{jazbec_generative_2025}.
...
\end{equation}
The noise schedule $\beta_t$ is chosen such that $\overline{\alpha}_T \to 0$, ensuring that at the final step $T$ the original signal is essentially destroyed and $x_{T}$ approximates the isotropic Gaussian prior $p_{init}(z) = \mathcal{N}(z; 0, I)$ \parencite{ho_denoising_2020}.
...
\label{eq:vlb_decomposition}
    \mathcal{L}_{VLB} = \mathbb{E}_q\Big[\, \underbrace{D_{KL}\!\big(q(x_T|x_0)\,\|\,p(x_T)\big)}_{L_T} \\
