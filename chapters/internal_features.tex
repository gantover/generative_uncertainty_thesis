...
% The fact that $t=0$ seemed to be the best performing timestep to compute the uncertainty is even more encouraging as it implies that we can use already 
...
A deeper investigation of the relation between
...
% We've however observed that this signal was sufficient to capture a meaningful epistemic uncertainty signal. But it would be worthwhile to perturb the weights of the internal UNET layer selected for the encoder to strengthen the uncertainty signal and improve the method further.

% \paragraph{Timestep $t=0$}
% Lastly, we've tried timesteps in the range $[45,5]$ but the observations suggests that better results could be obtained at $t=0$.
