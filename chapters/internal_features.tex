...
Although doing so would completely depart from the Bayesian framework, it would still provide valuable uncertainty measurement that are directly linked to the model's training manifold. Testing an internal encoder with \acs{DNI} is left as future work. This direction has close analogues emerging in the language-model literature, where perturbing internal representations and reading their dispersion is used to detect hallucinations; we are not aware of an equivalent for diffusion-image generation, which leaves it as genuinely open ground.

Still, Bayesian framework should not be discarded, as it is also likely that heavy approximations made the Bayesian uncertainty signal very weak. Improving the posterior approximation significantly with methods such as \cite{gegenfurtner_reducing_2026} or \cite{gupta_quantifying_2026} to see if we can get a useful bayesian uncertainty signal back, strong enough to survive the encoder bottleneck, is also left as future work.
...
\paragraph{Perturbation and variance as an uncertainty signal}
The \acs{DNI} mechanism, perturb an input and read the variance or entropy of a downstream representation, is a long-standing idea. It underlies test-time augmentation, where inputs are perturbed at inference and the spread of the resulting predictions is taken as an uncertainty estimate \parencite{wang_aleatoric_2019}, originally for discriminative tasks rather than generative-sample quality. 

Closer to our domain, \cite{vita_diffusion_2024} filter diffusion samples using the variance of the denoising scores under a diffusion-specific perturbation. The crucial distinction from \acs{DNI} lies in the representation in which the variance is read: \cite{vita_diffusion_2024} measure it in \textit{score space}, the U-Net's full image-dimensional noise prediction, and never use the network's internal feature activations, whereas \acs{DNI} reads it in a low-dimensional semantic-embedding space (an external encoder in \autoref{ch:semantic-encoder} or the model's own internal features in this chapter).

It should noted that \cite{vita_diffusion_2024} Aleatoric Uncertainty method was tried in \cite{jazbec_generative_2025} \acs{ADM} experiment and was found to be far less effective than the \acs{GU} method (and by extension our internal encoder method with \acs{LLLA}) on the \acs{FPR} metrics.

\paragraph{Internal diffusion features as a representation}
Using internal layers of a diffusion model as semantic representations is not a new idea \parencite{baranchuk_label-efficient_2022} argue that those that those layers can ``potentially be used as image representations for downstream tasks'' in their case they demonstrate their claim with a segmentation downstream task. 

More recently, \cite{luo_diffusion_2024} go over the same idea but also explore how features spread across layers and timesteps. In our \autoref{sub:internal-enc-abla-exp} ablation, we only try to find the best combo but leave the question of whether multiple layers/timesteps can be combined for future work.
